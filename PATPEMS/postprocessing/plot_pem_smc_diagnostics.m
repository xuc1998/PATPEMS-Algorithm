...
title('(a) \beta schedule','Interpreter','tex'); grid on;
xlim([1,K_plot]); ylim([0,1]);

% (2) ESS with threshold (dynamic if Policy='ESS' and Beta provided)
nexttile;
...
if drawThr && isfield(opts,'Resample') && isfield(opts.Resample,'Policy') ...
    ...
    band0 = [0.70 0.85];
    if isfield(opts,'Beta') && isfield(opts.Beta,'TargetBand') ...
            && ~isempty(opts.Beta.TargetBand)
        band0 = sort(opts.Beta.TargetBand(:)).';
    end

    % -------- build min(current band) for each plotted stage (respect BandDecay) --------
    useDecay = isfield(opts,'Beta') && isfield(opts.Beta,'BandDecay') ...
               && isfield(opts.Beta.BandDecay,'Enabled') ...
               && opts.Beta.BandDecay.Enabled;
    ...
    if useDecay
        ...
        if isfield(opts.Beta.BandDecay,'Floor') && ~isempty(opts.Beta.BandDecay.Floor)
...
xlabel(xLabelStr); ylabel('ESS'); title('(b) ESS (pre-resample)');
...
xlabel(xLabelStr); ylabel('rCESS'); title('(c) Relative CESS = CESS/N_p');
xlim([1,K_plot]); ylim([0,1]);
...
title('(e) Resampling (1 = yes)'); ylim([0,1.1]); xlim([1,K_plot]);

% (6) logZ cumulative
nexttile;
plot(stages, logZc, '-', 'LineWidth', 1.5); grid on;
xlabel(xLabelStr); ylabel('log Z (cum)'); title('(f) Cumulative log-evidence');
xlim([1,K_plot]);
