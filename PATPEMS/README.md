# PATPEMS — MATLAB Package

PATPEMS (Parallel Adaptive Transition Particle Evolution Metropolis Sequential Monte Carlo (PATPEMS))  
is a SMC sampler with **P**arallelism, **A**daptive **T**ransition of intermediate distributions, and flexible **PEM**
...
> `PATPEMS.m`, but it implements the PATPEMS algorithm described in the paper.

---

## 1. Overview

PATPEMS implements an annealed / tempered SMC sampler in MATLAB.  
...
2. **Adaptive Beta (AT)** – chooses sequence of intermediate distribution so that rCESS stays inside
...
/PATPEMS/
  ...
  /core/
    PATPEMS.m          % main sampler (public API, implements PATPEMS)
    install_PATPEMS.m  % add core / postprocessing to path
...
- `PATPEMS.m` – orchestrates Beta, (re)weighting, resampling, and move operators (ARM / XOVER / DE–MH).  
- `install_PATPEMS.m` – adds `core/` (and optionally `postprocessing/`) to the MATLAB path.  
...
```matlab
>> cd /PATPEMS/core/install_PATPEMS
```
...
```matlab
[parameter_iteration, out] = PATPEMS(Np, S, bound, logpdf_handle, options)
```
...
- `options` *(optional struct)* – algorithm settings (see header of `PATPEMS.m`)
...
Detailed field names and defaults are documented at the top of `core/PATPEMS.m`.  
Here is a minimal example:

```matlab
opts = struct();

% Beta: rCESS-band adaptive sequence schedule
...
```matlab
[traj, out] = PATPEMS(Np, S, bound, @(th) target(th, data), opts);
```
...
```matlab
>> cd my_example
>> my_example   % calls PATPEMS internally
```
...
>> [traj, out] = PATPEMS(2000, 800, bounds, logpdf, opts);
```
...
PATPEMS paper / preprint describing the algorithm and case studies.
