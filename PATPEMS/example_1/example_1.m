...
S    = 800;                  % max Beta stages if adaptive Beta
...
opts.Beta.TargetBand       = [0.75, 0.85];    % keep rCESS high early:small dBeta
opts.Beta.MinDeltaBeta     = 1e-3;          % absolute lower bound on step size
opts.Beta.MaxDeltaBeta     = 0.1;           % absolute upper bound on step size
opts.Beta.GrowthFactor     = 1.1;          % cap: dBeta_next <= 1.25 * dBeta_prev
opts.Beta.WarmupNoResample = 5;             % forbid resampling for the first K stages
...
%---------------------------
% Batch loop: run PATPEMS with different random seed
%---------------------------
